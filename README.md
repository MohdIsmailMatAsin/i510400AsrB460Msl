...
<p align="center">The USBMap.kext version I use is the same as the one generated by USBTollbox.  The injection step is performed using Windows 11 with the native usb port function.  In addition, the info.plist that contains the USBMap.kext file is a special modification including "XHCI-unsupported" injection used in previous versions of 100, 200, and 300 series USB devices due to no MacOS support for "8086A3AF" devices from the 400 series  USB device.</p>
